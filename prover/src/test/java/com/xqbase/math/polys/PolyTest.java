...
public class PolyTest {
	...
	public void testParse() {
		...
		Assert.assertEquals(A, "" + new Poly(VARS, A));
		Assert.assertEquals(D, "" + new Poly(VARS, D));
		Assert.assertEquals(E, "" + new Poly(VARS, E));
		Assert.assertEquals(F, "" + new Poly(VARS, F));
		Assert.assertEquals(X, "" + new Poly(VARS, X));
		Assert.assertEquals(Y, "" + new Poly(VARS, Y));
		Assert.assertEquals(Z, "" + new Poly(VARS, Z));
	}
	...
	public void testOnCircle() {
		Point p = P(X, Y, Z);
		Circle c = new Circle(new Poly(VARS, A), new Poly(VARS, D), new Poly(VARS, E), new Poly(VARS, F));
	...
	public void testDet() {
		...
		for (int i = 0; i < 4; i ++) {
			for (int j = 0; j < 4; j ++) {
				m[i][j] = new Poly(16, "" + (char) ('a' + i * 4 + j));
			}
		}
		...
		Poly x_G = new Poly(19, "-a**2*h*n*p + a**2*j*m*p + a*b*g*n*p - a*b*j*k*p - a*c*g*m*p + a*c*h*k*p - a*d*h*n*q + a*d*j*m*q - a*e*j*k*q + a*f*h*k*q + b*d*g*n*q - b*f*g*k*q - c*d*g*m*q + c*e*g*k*q");
		Poly y_G = new Poly(19, "-a*b*h*n*p + a*b*j*m*p - a*e*h*n*q + a*f*h*m*q + b**2*g*n*p - b**2*j*k*p - b*c*g*m*p + b*c*h*k*p + b*d*j*m*q + b*e*g*n*q - b*e*j*k*q - b*f*g*m*q - c*d*h*m*q + c*e*h*k*q");
		Poly z_G = new Poly(19, "-a*c*h*n*p + a*c*j*m*p - a*e*j*n*q + a*f*j*m*q + b*c*g*n*p - b*c*j*k*p + b*d*j*n*q - b*f*j*k*q - c**2*g*m*p + c**2*h*k*p - c*d*h*n*q + c*e*g*n*q - c*f*g*m*q + c*f*h*k*q");
		Poly x_H = new Poly(19, "-a*d*h*n*p*s + a*d*j*m*p*s + a*e*g*j*p*r + a*e*g*n*p*s - a*f*g*h*p*r - a*f*g*m*p*s - b*d*g*j*p*r - b*d*j*k*p*s + b*f*g**2*p*r + b*f*g*k*p*s + c*d*g*h*p*r + c*d*h*k*p*s - c*e*g**2*p*r - c*e*g*k*p*s - d**2*h*n*q*s + d**2*j*m*q*s + d*e*g*n*q*s - d*e*j*k*q*s - d*f*g*m*q*s + d*f*h*k*q*s");
		Poly y_H = new Poly(19, "a*e*h*j*p*r + a*e*j*m*p*s - a*f*h**2*p*r - a*f*h*m*p*s - b*d*h*j*p*r - b*d*h*n*p*s + b*e*g*n*p*s - b*e*j*k*p*s + b*f*g*h*p*r + b*f*h*k*p*s + c*d*h**2*p*r + c*d*h*m*p*s - c*e*g*h*p*r - c*e*g*m*p*s - d*e*h*n*q*s + d*e*j*m*q*s + e**2*g*n*q*s - e**2*j*k*q*s - e*f*g*m*q*s + e*f*h*k*q*s");
		Poly z_H = new Poly(19, "a*e*j**2*p*r + a*e*j*n*p*s - a*f*h*j*p*r - a*f*h*n*p*s - b*d*j**2*p*r - b*d*j*n*p*s + b*f*g*j*p*r + b*f*g*n*p*s + c*d*h*j*p*r + c*d*j*m*p*s - c*e*g*j*p*r - c*e*j*k*p*s - c*f*g*m*p*s + c*f*h*k*p*s - d*f*h*n*q*s + d*f*j*m*q*s + e*f*g*n*q*s - e*f*j*k*q*s - f**2*g*m*q*s + f**2*h*k*q*s");
