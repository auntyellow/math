...
def main():
    ...
    # U = S(9)/2 can guarantee g = a - (b + c)/11 >= 0
    U = S(9)/2
    # a <= b <= c <= (1 + U)*a
    print('f(abcU) =', factor(f.subs(c, a*(1 + U/(1 + u))).subs(b, a*(1 + U/(1 + u + v)))))
    print('g(abcU) =', factor(g.subs(c, a*(1 + U/(1 + u))).subs(b, a*(1 + U/(1 + u + v))).subs(m, m0)))
    # c <= b <= a <= (1 + U)*c
    print('f(cbaU) =', factor(f.subs(a, c*(1 + U/(1 + u))).subs(b, c*(1 + U/(1 + u + v)))))
    print('g(cbaU) =', factor(g.subs(a, c*(1 + U/(1 + u))).subs(b, c*(1 + U/(1 + u + v))).subs(m, m0)))
    # b <= a <= c <= (1 + U)*b
    print('f(bacU) =', factor(f.subs(c, b*(1 + U/(1 + u))).subs(a, b*(1 + U/(1 + u + v)))))
    print('g(bacU) =', factor(g.subs(c, b*(1 + U/(1 + u))).subs(a, b*(1 + U/(1 + u + v))).subs(m, m0)))
    print()
    # f(1,0,0) = 0
    f = f0**2 - (g/h)**2
